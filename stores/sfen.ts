...
export const useSfenStore = defineStore('sfen', {
  ...
  actions: {
    setText(payload: { text: string }) {
      this.text = payload.text
    },

    receiveText(payload: { text: string }) {
      this.text = payload.text
    },

    setRoomId(payload: { roomId: string }) {
    ...
    reverse() {
      // 盤面を180度回転
      const newRows: Board = Array(9).fill(null).map(() => Array(9).fill(null))
    ...
    moveBoardToBoard(payload: { beforeX: number, beforeY: number, afterX: number, afterY: number }) {
    ...
    moveBoardToHand(payload: { beforeX: number, beforeY: number, afterX: string, afterY: string }) {
      this.rows[payload.beforeY][payload.beforeX] = null
    },

    moveHandToBoard(payload: { beforeX: string, beforeY: string, afterX: number, afterY: number }) {
      this.rows[payload.afterY][payload.afterX] = payload.beforeY
      this.latestX = payload.afterX
      this.latestY = payload.afterY
    },

    moveHandToHand(payload: { beforeX: string, beforeY: string, afterX: string, afterY: string }) {
      // 手駒から手駒への移動（実装省略）
    },

    moveBoardToStock(payload: { beforeX: number, beforeY: number }) {
      this.rows[payload.beforeY][payload.beforeX] = null
    },

    moveStockToBoard(payload: { piece: string, afterX: number, afterY: number }) {
      this.rows[payload.afterY][payload.afterX] = payload.piece
      this.latestX = payload.afterX
      this.latestY = payload.afterY
    },

    togglePromotedAndTurn(payload: { x: number, y: number }) {
