...
export function validateFieldValue(value: any): boolean {
...
export function setupSocketEvents(io: Server) {
  ...
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('enterRoom', (id: string) => {
      ...
      // Redisから部屋のデータを取得
      redis.get(getRoomKey(roomId), function(err: any, result: any) {
        if (err) {
    ...
    socket.on('sendComment', (params: any) => {
      // Validate params
      if (!params || typeof params !== 'object') {
